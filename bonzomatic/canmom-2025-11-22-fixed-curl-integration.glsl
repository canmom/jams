...
vec3 integrate(ivec2 UV, vec3 offset, vec3 noise) {
  ...
  vec3 noise_up = noised(pos+ offset).yzw + noised(2*pos + offset).yzw;
  vec3 noise_down = noised(-pos+offset).yzw + noised(-2*pos + offset).yzw;
...
void main(void) {
  ...
  out_color = vec4(vec3(dots) +0.5*vec3(0.12,0.04,0.12)*previous_frame+0.4*vec3(0.12,0.02,0.2)*previous_frame.yzx,1.0);
