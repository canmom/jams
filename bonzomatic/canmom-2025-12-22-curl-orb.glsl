...
//shaderjam 2025-12-22
//greets to reality (host), i.v. (music), aldroid, boris, marex, david, muffintrap!
//and everyone at fieldfx for a great first year in the demoscene!
...
vec3 integrate(ivec2 UV, vec3 offset, vec3 noise) {
  ...
  vec3 noise_pos = NOISE_SCALE*(pos-0.5);
  vec3 sphere_field = normalize(pos-0.5)*noised(noise_pos+ offset).x;
  float epsilon = 0.001;
  vec3 dx = vec3(1,0,0)*epsilon;
  vec3 dy = vec3(0,1,0)*epsilon;
  vec3 dz = vec3(0,0,1)*epsilon;
  vec3 sphere_dx = (normalize(pos-0.5+dx)*noised(noise_pos + dx + offset).x - sphere_field);
  vec3 sphere_dy = (normalize(pos-0.5+dy)*noised(noise_pos + dy + offset).x - sphere_field);
  vec3 sphere_dz = (normalize(pos-0.5+dz)*noised(noise_pos + dz + offset).x - sphere_field);
  ...
  float dist = dot(new_pos-0.5, new_pos-0.5);
  if (any(greaterThan(new_pos, vec3(1.0))) || any(lessThan(new_pos,vec3(0.0))) || dist>0.25 || (dist <0.2 && dist > 0.04)) {
...
void project_particle(vec3 projected_pos, float value) {
  ...
  imageAtomicAdd(computeTex[2],projected_pixel,write);
}
...
void main(void) {
  ...
  float fftTime3 = fftTimeScale*texture(texFFTIntegrated,0.15).x;
  ...
  if (h.z < 0.001) {
    ...
    particle_pos = integrate(UV, vec3(0.8*fftTime), 0.001*h);
  }
  
  
  float particle_radius = dot(particle_pos - 0.5, particle_pos - 0.5);
  ...
  float camera_distance = 1.5 + 0.4*cos(fftTime3);
  ...
  project_particle(projected_pos, step(particle_radius, 0.1));
  ...
  uint buff =imageLoad(computeTexBack[2],UV).r;
  vec3 dots = vec3(buff & 0xFFFF, (buff & 0xFFFF0000)>>16, 1.0)*0.03;
  //switch to this to disable the glow effect
  //out_color = vec4(vec3(dots),1.0);
  out_color = vec4(0.8*vec3(dots) +0.2*vec3(0.06,0.04,0.16)*previous_frame+0.2*vec3(0.3,0.3,0.5)*previous_frame.yzx + dot(uv,uv)*0.1,1.0);
