...
//a shader coded in Amsterdam airport
//because you can shaderjam anywhere there's wifi baby
...
//soon to be seen in the skies over k̦ln!
...
void main(void)
{
  ...
  vec2 cuv = (gl_FragCoord.xy /v2Resolution - 0.5)/vec2(v2Resolution.y/v2Resolution.x,1.)*2.0 + 0.5*vec2(3.0*cos(camera_time_x), 2.0*sin(camera_time_y));

  vec2 uv = vec2(atan(cuv.y, cuv.x)/(2*PI), length(cuv));
  ...
  float layer = floor((uv.y)*layer_height+1.0);

  uv.x *= (2.0/layer_height)*layer;
  ...
  uv += 0.4*(noise_curl.yz * texture(texFFTSmoothed,0.2).r + noise_curl.xy * texture(texFFTSmoothed,0.1).r);
  ...
  vec2 tunnel = 0.5+0.3*vec2(s,-c);
	...
	out_color = vec4(0.9*colour,1.0) + texture(texPreviousFrame, 1.01*(gl_FragCoord.xy/v2Resolution-tunnel)+tunnel)*0.5;
